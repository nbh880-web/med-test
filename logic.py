...
def create_pdf_report(summary_df, raw_responses):
    """מפיק דוח PDF מתוקן ללא שגיאות מרחב"""
    ...
    # הגדרת רוחב עבודה למניעת שגיאות horizontal space
    ...
    pdf.cell(effective_width, 20, txt=fix_heb("דוח הכנה למבחני מסר - HEXACO"), ln=True, align='C')
    
    pdf.set_font(font_main, size=12)
    curr_time = datetime.now().strftime("%d/%m/%Y %H:%M")
    pdf.cell(effective_width, 10, txt=fix_heb(f"תאריך ביצוע: {curr_time}"), ln=True, align='C')
    pdf.ln(10)
    ...
    pdf.cell(col_width, 12, fix_heb("תכונת אישיות"), 1, 0, 'C', True)
    pdf.cell(col_width, 12, fix_heb("ציון"), 1, 0, 'C', True)
    pdf.cell(col_width, 12, fix_heb("סטטוס יעד"), 1, 1, 'C', True)
    ...
    for _, row in summary_df.iterrows():
        ...
        pdf.cell(col_width, 10, fix_heb(str(row['trait'])), 1, 0, 'C')
        pdf.cell(col_width, 10, f"{score:.2f}", 1, 0, 'C')
        pdf.cell(col_width, 10, fix_heb(status), 1, 1, 'C')
    ...
    if inconsistencies:
        ...
        pdf.cell(effective_width, 15, txt=fix_heb("ניתוח סתירות פנימיות"), ln=True, align='R')
        pdf.ln(5)
        
        pdf.set_font(font_main, size=11)
        for pair in inconsistencies:
            pdf.set_text_color(200, 0, 0)
            pdf.cell(effective_width, 10, txt=fix_heb(f"סתירה בתכונת: {pair['trait']}"), ln=True, align='R')
            
            pdf.set_text_color(0, 0, 0)
            # שימוש ברוחב מוגדר ב-multi_cell פותר את השגיאה
            pdf.multi_cell(effective_width, 8, txt=fix_heb(f"שאלה 1: {pair['q1_text']} (תשובה: {pair['q1_ans']})"), align='R')
            pdf.multi_cell(effective_width, 8, txt=fix_heb(f"שאלה 2: {pair['q2_text']} (תשובה: {pair['q2_ans']})"), align='R')
            pdf.ln(2)
            pdf.line(pdf.get_x(), pdf.get_y(), pdf.get_x() + effective_width, pdf.get_y())
            pdf.ln(4)
