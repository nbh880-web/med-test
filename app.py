...
# הערה: וודא שהקבצים האלו קיימים אצלך בפרויקט
try:
...
st.markdown("""
    ...
    [data-testid="stSidebar"] * { word-break: normal !important; text-align: right; }
...
for key in ['step', 'responses', 'current_q', 'user_name', 'questions', 'start_time']:
...
def show_admin_dashboard():
    ...
    if not df.empty:
        selected_name = st.selectbox("בחר מועמד לפירוט:", df['user_name'].unique())
        user_row = df[df['user_name'] == selected_name].iloc[0]
        st.markdown(f'<div class="ai-report-box" style="border-right-color: #1e3a8a;">{user_row.get("ai_report", "אין דוח")}</div>', unsafe_allow_html=True)
...
if st.session_state.user_name == "adminMednitai" and st.session_state.step == 'ADMIN_VIEW':
            ...
            if history:
                for entry in history:
                    with st.expander(f"📅 מבדק מ-{entry.get('test_date', 'לא ידוע')}"):
                        st.write(entry.get('ai_report', 'אין דוח זמין'))
    ...
    if st.button("📥 הורד דוח PDF"):
        pdf = create_pdf_report(summary_df, df_raw)
        st.download_button("לחץ להורדה", pdf, f"Report_{st.session_state.user_name}.pdf")
